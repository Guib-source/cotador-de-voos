...
internal static class Regression
{
    ...
    public static int Main(string[] args)
    {
        try
        {
            ...
            Check(AirportCatalog.Search("slz").First().Code == "SLZ", "Buscar IATA sem diferenciar caixa.");
            Check(AirportCatalog.Search("sao luis").Any(a => a.Code == "SLZ"), "Buscar cidade sem acentos.");
            Check(AirportCatalog.Search("Congonhas").Any(a => a.Code == "CGH"), "Buscar nome do aeroporto.");
            Check(AirportCatalog.Search("sao paulo").Count > 1, "Preservar opções de cidade com vários aeroportos.");
            Check(AirportCatalog.Search("aeroporto inexistente xyz").Count == 0, "Busca sem resultado não escolhe aeroporto.");
            Check(AirportCatalog.Names.Count == Quote.Cities.Count && Quote.Cities.Keys.All(AirportCatalog.Names.ContainsKey), "Nomes cobrem todo o catálogo atual.");
            var corrected = Quote.Parse("1234 12 Nov 04:35h 12 Nov 07:05h VCP LOB", 2026)[0];
            Check(corrected.Notices.Count == 1 && corrected.Notices[0].Column == 1 && corrected.Notices[0].Message.Contains("LOB corrigido para LDB"), "Rastrear a correção de Londrina no destino.");
            var correctedOrigin = Quote.Parse("1234 12 Nov 04:35h 12 Nov 07:05h CCH SDU", 2026)[0];
            Check(correctedOrigin.Notices.Any(n => n.Column == 0 && n.Value == "CGH"), "Rastrear correção CGH na origem.");
            var correctedLisbon = Quote.Parse("1234 12 Nov 04:35h 12 Nov 07:05h GRU - SAO PAULO US - LISBOA", 2026)[0];
            Check(correctedLisbon.Notices.Any(n => n.Value == "LIS"), "Rastrear Lisboa com código de duas letras.");
            var connections = Quote.Group(Quote.Parse("1234 12 Nov 04:35h 12 Nov 07:05h SLZ CRU\n1235 12 Nov 09:00h 12 Nov 11:00h GRU BSB", 2026));
            Check(connections[0].Notices.Any(n => n.Column == 7 && n.Message.Contains("CRU corrigido para GRU")), "Preservar aviso de aeroporto interno na conexão agrupada.");
            var reviewValues = new[] { "VCP", "LDB", "12/11/26", "12/11/26", "04:35", "07:05", "", "Voo direto" };
            var warnings = FlightReview.Inspect(reviewValues, corrected.Notices);
            Check(warnings.ContainsKey(1) && warnings.ContainsKey(6), "Avisar correção e companhia ausente.");
            reviewValues[1] = "BSB"; reviewValues[6] = "LATAM";
            Check(FlightReview.Inspect(reviewValues, corrected.Notices).Count == 0, "Não manter aviso incorreto depois de edição manual.");
            reviewValues[1] = "XYZ"; reviewValues[2] = "31/02/26"; reviewValues[4] = "25:00";
